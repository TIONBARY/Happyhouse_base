...
| HouseServiceImpl 34      | @Override<br/>	public HouseDeal search(int no) {<br/>		<br/>		// complete code #03<br/>		// null 을 return 하면 안됩니다. Dao Layer 의 적절한 method를 호출하여 Business Logic 을 완성하세요.<br/>		```public HouseDeal search(int no) {<br/> for (int i = 0; i < search.size(); i++) {<br/> if (no == search.get(i).getNo()) {<br/> return search.get(i);<br/> }<br/> }<br/> return null;<br/> }<br/>```
	<br/>	 |
| HouseDaoImpl  80         | 못함                                                         |
